/* AirBond v2 — main.js  (clean, no duplicates) */

/* ── Hide float-btns on touch devices ── */
...
const aiFloatBtn = document.getElementById('aiFloatBtn'); // может отсутствовать
if (aiFloatBtn && aiBubble) {
  aiFloatBtn.addEventListener('click', () => aiBubble.classList.toggle('open'));
}

function scrollToSection(id) {
  if (aiBubble) aiBubble.classList.remove('open');
...
/* ── Header shadow on scroll (class only, no inline styles) ── */
...
/* ── Stats countUp — safe for text with symbols ── */
const countUpObserver = new IntersectionObserver((entries) => {
  entries.forEach(e => {
    ...
    /* Find numeric part and preserve prefix/suffix */
    const match = raw.match(/^([^0-9−\-]*)([0-9]+)(.*)$/);
    if (!match) return; // no clean number → skip animation
...
/* ── Appointment timer (next working day +2) ── */
(function() {
